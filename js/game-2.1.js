...
var mainChain = function() {
    ...
    var jioConf = { "autoControl": ["volume", "exit"], "gameName": "Test FP", "gameVersion": "1.0.1" };
	...
	var adSpot = "unique_ad_1";
	var adSpotRewarded = "testfp";
	var isReady= false;
    ...
    window.cacheAds = function() {
        ...
        // VMAX.jioSDK_adId = "midroll"; // <ins ads id in index.html
        // VMAX.jioSDK_adId = "unique_ad_1"; // <ins ads id in index.html
        // VMAX.jioSDK_adId = "testfp"; // <ins ads id in index.html
		...
		// else
		{
			if(isReadyRewarded == false)
			{
				VMAX.cacheAd(adSpotRewarded);
			}
		}
        VMAX.onAdReady = function(AdPlacementId) {
			...
			if(AdPlacementId == adSpotRewarded)
			{
				isReadyRewarded = true;
				console.log("VMAX: onAdReady adSpotRewarded");
			}
        ...
        VMAX.onAdError = function(AdPlacementId, errorCode) {
			...
			if(AdPlacementId == adSpotRewarded)
			{
				isReadyRewarded = false;
				console.log("VMAX: onAdError adSpotRewarded");
			}
        }
    ...
    window.showAds = function() { // use this showAds func in your game levels/game over or maintain your ad frequency when to show ads
		...
		// else
		{
			if(isReadyRewarded == true)
			{
				VMAX.showAd(adSpotRewarded);
			}
		}
    }
